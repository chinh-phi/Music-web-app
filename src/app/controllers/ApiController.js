...
class ApiController {
    ...
    async singerCategoryDetails(req, res, next) {
        const singerNameQuery = req.params.singer;
        const singer = await Singer.findOne({slug: singerNameQuery});
        const listSongOfSinger = singer.songs;
        const category = await Category.findOne({slug: req.params.slug});
        const listSongOfCategory = category.songs;
        const arrData = listSongOfCategory.filter((value => {
            return listSongOfSinger.includes(value);
        }))

        // Kiểm tra xem bài hát có nằm trong favorite track hay ko
        const checkSong = [];
        const user = await User.findOne({ _id: req.session.user._id });
        const favoriteTrack = await FavoriteTrack.findOne({ favoriteTrackId: user.favoriteTrackId });
        const fTrackId = favoriteTrack.songId;
        for (let i = 0; i < arrData.length; i++) {
            checkSong[i] = false;
            if (fTrackId.includes(arrData[i])) {
                checkSong[i] = true;
            }
        }

        const data = await Song.find({songId: arrData});

        // Xử lý lấy tên ca sĩ
        var singerArr = [];
        var dataArr = [];
        for (let i = 0; i < data.length; i++) {
            const { _id, songId, title, Lyric, path, duration, img, singers, createdAt, updatedAt } = data[i];
            const check = checkSong[i];
            if (data[i].singers.length == 1) {
                let singer = await Singer.findOne({ singerId: data[i].singers[0] });
                const singerName = singer.name;
                const x = { _id, songId, title, Lyric, path, duration, img, singers, createdAt, updatedAt, singer, singerName, check }
                dataArr.push(x);
            } else {
                ...
                for (let j = 0; j < data[i].singers.length; j++) {
                    let singer = await Singer.findOne({ singerId: data[i].singers[j] });
                    singerArr.push(singer);
                    singerName.push(singer.name);
                }
                const singer = singerArr;
